<!--
  创建时间: 2026-06-03
  文件名: RAG基础.md
  文件描述: RAG基础概念与架构详解，补充企业级方案判断、评测闭环与验收标准
...
**能力对标**：本章对应 [能力模型](../00-Roadmap/能力模型.md) 中「AI应用构建力 → 知识库构建能力」核心能力，是整个 RAG 模块的基础章节。学完本章后，你应能从产品与工程双视角解释 RAG 的价值、链路与适用边界。
...
### 1.3 企业级场景中何时优先考虑 RAG

| 场景特征 | 是否优先考虑 RAG | 原因 |
|------|----------------|------|
| **知识频繁更新** | 是 | 更新知识库比重新训练更快 |
| **需要引用与溯源** | 是 | RAG 天然适合返回来源 |
| **依赖私有知识** | 是 | 可把企业文档接入检索链路 |
| **只想改变表达风格** | 否 | 更适合 Prompt 或微调 |
| **主要问题是执行动作不是知识问答** | 视情况 | 更可能需要 Agent 或工作流 |
...
企业级 RAG 评测不能只看“模型回答像不像”，还要区分以下 3 层：

1. **检索层**：有没有把真正相关的内容找出来
2. **生成层**：回答是否忠实于检索内容、是否有引用与幻觉
3. **业务层**：用户是否真正解决问题，是否减少人工成本或提高效率
...
| **高频问题集** | 验证主流场景是否稳定可用 |
| **长尾问题集** | 验证边缘查询和复杂表达 |
| **无答案问题集** | 验证系统能否明确说“不知道” |
| **高风险问题集** | 验证权限、敏感信息与合规边界 |

### 4.2 常见评测误区

| 误区 | 表现 | 正确做法 |
|------|------|---------|
| 只看最终回答 | 忽略检索阶段是否命中 | 检索与生成分层评测 |
| 只看平均分 | 少数严重坏例子被掩盖 | 单独维护 Bad Case 集 |
| 只测知识库内问题 | 无法验证拒答能力 | 加入无答案与越权样本 |
| 只在线上看反馈 | 问题定位慢 | 建立离线评测与线上监控联动 |
...
#### 5.1.1 判断一个业务问题是否适合用 RAG 的 5 个问题

| 问题 | 说明 |
|------|------|
| 是否依赖外部或私有知识 | 如果答案主要来自文档或知识库，RAG 价值更高 |
| 知识是否经常变化 | 更新越频繁，RAG 越优于静态模型记忆 |
| 是否需要引用来源 | 有审计、可信或解释要求时优先考虑 RAG |
| 是否存在权限隔离 | RAG 可结合元数据与权限控制做差异化返回 |
| 是否能接受检索带来的额外延迟 | 如果极端延迟敏感，需要更谨慎设计 |

### 5.2 企业级产品化关注点

| 维度 | 重点关注内容 |
|------|-------------|
| **知识范围** | 文档类型、规模、更新频率、多语言支持 |
| **查询场景** | 单轮问答、多轮问答、复杂分析、图文混合 |
| **风险治理** | 敏感信息过滤、权限控制、来源展示、无答案处理 |
| **用户体验** | 引用高亮、原文查看、追问建议、反馈入口 |
| **成本治理** | Embedding、检索、重排序和生成的综合成本 |

### 5.3 RAG 常见失败模式

| 失败模式 | 根因 | 优先排查方向 |
|------|------|-------------|
| 明明有答案却没答出来 | 分块或召回失败 | Chunk 粒度、Embedding、Recall@K |
| 回答看起来像对但引用不准 | 上下文构建有问题 | 去重、截断、引用映射 |
| 回答引用到了不该看的内容 | 权限过滤缺失 | 元数据过滤与访问控制 |
| 回答太慢 | 检索链路过长 | top_k、重排序、缓存、模型选择 |
...
- [ ] 能解释 RAG 解决的核心问题，以及它与微调、Prompt 工程的边界
- [ ] 能画出一条完整 RAG 链路，并说明各组件职责
- [ ] 能从效果、延迟、成本、风险四个维度判断业务场景是否适合用 RAG
- [ ] 能提出基础的评测集设计与 Bad Case 复盘方法

### 6.2 进阶标准

- [ ] 能为一个真实业务场景设计基础 RAG 架构和评测方案
- [ ] 能识别权限、无答案处理、引用展示等企业级关键要求
- [ ] 能把 RAG 能力映射到客服、搜索、Copilot 或知识助手产品方案中
