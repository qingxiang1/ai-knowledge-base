<!--
  创建时间: 2026-06-03
  文件名: HybridSearch.md
  文件描述: RAG混合搜索详解，补充企业级混合检索决策框架、治理重点与验收标准
...
**能力对标**：本章对应 [能力模型](../00-Roadmap/能力模型.md) 中「AI应用构建力 → 知识库构建能力」与「技术理解力 → 检索链路理解」。掌握混合搜索，意味着你能判断什么时候需要多路召回、如何平衡效果与延迟，以及如何把检索能力做成稳定的企业级产品能力。
...
### 1.3 企业级场景下为什么常常需要混合搜索

企业场景中的查询远比 Demo 复杂，常见问题包括：

1. **用户表达不稳定**：同一个意图可能有口语、术语、缩写、错别字等多种表达
2. **知识内容形态混杂**：既有制度文档，也有 FAQ、工单、产品说明和结构化字段
3. **业务既要语义理解，也要精确命中**：例如型号、合同编号、错误码、药品名都要求精确匹配
4. **单路检索存在天然短板**：只靠向量检索容易漏掉关键词，只靠关键词检索又容易错过语义相关内容

因此，企业级混合搜索的核心不是“把更多技术拼在一起”，而是通过多路召回与合理融合，把不同类型问题都尽可能稳定地接住。
...
#### 7.1.1 决定是否引入混合搜索前先回答 5 个问题

| 问题 | 说明 |
|------|------|
| 查询里是否大量出现专有名词、型号、错误码 | 如果是，通常需要保留关键词或稀疏检索能力 |
| 用户表达是否高度多样 | 如果表达波动大，仅靠关键词常常不够 |
| 当前问题是“没召回”还是“排不准” | 前者优先补召回，后者再看 Rerank |
| 延迟预算是否支持多路并发与融合 | 多路检索和精排会增加时延与复杂度 |
| 是否存在权限、租户、标签等过滤条件 | 混合搜索必须和 metadata 过滤协同设计 |

#### 7.1.2 常见选型建议

| 场景 | 建议方案 |
|------|----------|
| 中小规模中文知识库 | 稠密向量 + BM25 + RRF |
| 专有名词很多的企业文档 | 稠密向量 + 稀疏检索 + 实体词典增强 |
| 高价值问答或 Copilot 场景 | 多路召回 + Rerank + Bad Case 闭环 |
| 极度关注延迟的轻量场景 | 优先双路混合，不宜一开始堆太多渠道 |

### 7.2 核心指标

企业级混合搜索通常至少同时看 4 类指标：

1. **召回指标**：Recall@K、覆盖率、候选集中正确答案出现率
2. **排序指标**：MRR、NDCG、Precision@K
3. **性能指标**：P50、P95、P99 延迟，多路超时率，降级触发率
4. **业务指标**：答案采纳率、人工转接率、问题解决率、用户满意度

### 7.3 混合搜索治理重点

1. **渠道治理**：明确每一路召回解决什么问题，避免无意义堆渠道
2. **权重治理**：权重与融合参数要基于评测集迭代，而不是靠经验拍脑袋
3. **查询治理**：改写、扩展、分解不能无上限放大噪声
4. **过滤治理**：权限、租户、时间范围、文档状态要在召回链路中稳定生效
5. **降级治理**：多路超时或某一路异常时，要能退回可接受的默认方案
6. **评测治理**：持续维护“关键词必须命中”“语义必须召回”“多跳问题需分解”等样本集

### 7.4 常见失败模式

1. **渠道越多越好**：导致系统复杂度上升，但收益并不明显
2. **融合参数长期不调**：知识结构和查询分布变化后，旧参数会失效
3. **查询改写过度**：把原始意图改坏，导致召回结果跑偏
4. **忽略权限过滤**：技术上检索成功，但业务上形成越权风险
5. **只看离线指标**：上线后真实查询分布变了，效果并不稳定

## 八、企业级验收标准

### 8.1 学完本章后至少应做到

- [ ] 能解释为什么单一路径检索无法覆盖大多数企业真实场景
- [ ] 能根据查询特征、知识形态和延迟预算初步判断是否需要混合搜索
- [ ] 能理解 RRF、加权融合、多路召回与 Rerank 的边界关系
- [ ] 能识别查询治理、过滤治理、降级治理对混合搜索稳定性的影响
- [ ] 能从效果、性能、成本和风险四个维度评估混合搜索方案
