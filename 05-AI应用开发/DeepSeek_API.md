<!--
  创建时间: 2026-06-03
  文件名: DeepSeek_API.md
  文件描述: DeepSeek API 使用指南，补充企业接入、性价比治理与验收清单
...
- 理解 DeepSeek-V3 与 R1 在 API 接入和任务适配上的差异
- 为企业场景设计 DeepSeek 的高性价比接入与模型路由方案
- 处理 JSON Mode、函数调用、推理链输出和错误治理
- 评估 DeepSeek 在国产替代、预算控制和能力组合上的价值
- 输出一份 DeepSeek API 的接入和降本方案
...
### 8.1 DeepSeek API 接入检查表

| 设计项 | 关键问题 | 输出物 |
| ------ | -------- | ------ |
| 模型定位 | 当前场景适合 V3 还是 R1？ | 模型定位说明 |
| 降本目标 | DeepSeek 预期替代哪些高成本模型？ | 成本替代分析 |
| 能力边界 | 多模态缺口、函数调用支持差异是否已评估？ | 能力边界说明 |
| 路由策略 | 哪些请求走 DeepSeek，哪些回退其他模型？ | 路由规则 |
| 稳定性 | 限流、超时、版本变化如何治理？ | 稳定性方案 |
...
```json
{
  ...
  "routing_policy": {
    "general_qa": "deepseek-chat",
    "complex_reasoning": "deepseek-reasoner"
  },
  "cost_goal": "reduce_model_cost_by_60_percent",
  "timeout_ms": 20000
}
```
...
| 误区 | 问题 | 正确做法 |
| ---- | ---- | -------- |
| 只因为便宜就全量替换 | 忽略能力边界和生态差异 | 先做分场景替换和 POC |
| 把 R1 当万能模型 | 推理模型延迟更高且不适合所有任务 | 仅用于复杂推理和高价值场景 |
| 忽略 reasoning token 成本 | 预算预估失真 | 单独统计推理链相关消耗 |
| 不做多模型回退 | 某些能力缺口无法覆盖 | 与 GPT/Qwen 等形成组合方案 |
| 只看 API 成本 | 忽视研发、测试和治理成本 | 做总拥有成本评估 |

---

## 十、阶段验收标准

- [ ] 能完成 DeepSeek API 的基础接入与模型路由设计
- [ ] 能区分 V3 与 R1 的适用场景和成本结构
- [ ] 能设计国产替代或降本目标下的接入方案
- [ ] 能输出一份 DeepSeek API 的预算、路由与回退策略
...
- **2026-06-03** 初版完成，涵盖 DeepSeek API、JSON Mode、R1 推理与最佳实践
