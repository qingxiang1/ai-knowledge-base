<!--
  创建时间: 2026-06-03
  文件名: Rerank.md
  文件描述: RAG重排序策略详解，补充企业级选型框架、上线治理与验收标准
...
**能力对标**：本章对应 [能力模型](../00-Roadmap/能力模型.md) 中「AI应用构建力 → 知识库构建能力」核心能力。掌握 Rerank，意味着你能在召回候选集基础上进一步提升相关性，并在效果、延迟、成本之间做企业级取舍。
...
| 场景信号 | 是否建议引入 Rerank | 原因 |
|------|------------------|------|
| **召回结果里有相关内容，但排序不稳定** | 是 | Rerank 最擅长解决“有但排不前” |
| **知识规模增大，召回噪声明显上升** | 是 | 需要精排做二次筛选 |
| **延迟极度敏感且召回已足够稳定** | 视情况 | Rerank 可能带来额外时延 |
| **候选集本身就严重缺失答案** | 否 | 应先优化召回而不是上来精排 |
...
#### 6.1.1 决定是否上 Rerank 前要回答 4 个问题

| 问题 | 说明 |
|------|------|
| 候选集里是否已经常常包含正确答案 | 如果没有，应先优化召回 |
| 业务对精度提升是否足够敏感 | 高价值场景更值得投入精排 |
| 延迟预算能否容纳额外排序开销 | 精排不是免费午餐 |
| 成本预算是否允许增加模型调用 | 尤其是 API 或 LLM 型精排 |

#### 6.1.2 常见选型建议

| 场景 | 推荐方案 |
|------|---------|
| **简单场景** | 不上 Rerank 或轻量模型 |
| **中等复杂度** | MiniLM / BGE-base 等轻量精排 |
| **高要求中文场景** | BGE-Reranker 等中文优化模型 |
| **多语言和快速接入** | Cohere Rerank API |
| **极致效果场景** | LLM-based Rerank，但需严格控成本 |

### 6.2 核心指标

| 指标 | 关注点 |
|------|------|
| **NDCG@K / MRR 提升** | 是否真的把更相关内容排到前面 |
| **P99 延迟增加** | 是否在业务可接受范围内 |
| **吞吐量影响** | 是否影响整体系统容量 |
| **单次查询成本** | 是否值得为提升买单 |
| **人工满意度提升** | 是否真正改善最终答案体验 |

### 6.3 上线治理建议

1. **先离线评测**：确认精排确实带来排序收益
2. **再灰度发布**：先给小流量或高价值场景启用
3. **监控坏例子**：重点关注“排序更差了”的退化样本
4. **保留回滚开关**：Rerank 异常时可快速退回召回直出模式

### 6.4 常见失败模式

| 失败模式 | 表现 | 优先排查方向 |
|------|------|-------------|
| 精排收益不明显 | 指标提升小于预期 | 候选集质量、模型匹配度、阈值设置 |
| 延迟显著上升 | 用户响应明显变慢 | 批处理、缓存、候选集规模、模型大小 |
| 成本失控 | 调用费用或 GPU 成本超预算 | 降低候选集、换轻量模型、分层精排 |
| 好内容被错杀 | 结果更“干净”但覆盖下降 | 阈值过高、排序模型偏置、样本不足 |
...
- [ ] 能解释为什么召回后还需要 Rerank，以及它与召回的边界
- [ ] 能根据延迟预算、成本预算和效果目标初步选择精排方案
- [ ] 能理解交叉编码器、API 型精排和 LLM 型精排的取舍
- [ ] 能识别“召回有答案但排序不准”这类适合引入 Rerank 的典型场景

### 7.2 进阶标准

- [ ] 能为一个真实业务场景设计 Rerank 上线方案、灰度策略与回滚预案
- [ ] 能建立精排离线评测、线上监控和退化 Bad Case 复盘机制
- [ ] 能把效果、延迟、成本三者纳入统一的产品决策框架
