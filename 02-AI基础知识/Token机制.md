...
#### 5.6.1 上线后建议持续监控的 Token 指标

| 指标                         | 作用                             |
| ---------------------------- | -------------------------------- |
| 单次请求平均 Token           | 观察常规请求成本水平             |
| P95 / P99 Token              | 识别长尾高成本请求               |
| 输入/输出 Token 比           | 判断问题出在输入膨胀还是输出冗长 |
| 系统 Prompt 占比             | 判断是否存在固定成本过高问题     |
| 按用户/功能维度的 Token 消耗 | 支撑额度策略和商业化分层         |

#### 5.6.2 企业最常见的治理动作

1. 对高成本场景启用摘要、裁剪和缓存
2. 为不同用户层级配置不同模型与额度
3. 对异常 Token 请求做告警和熔断
4. 在版本评审中同步看效果、成本和延迟
