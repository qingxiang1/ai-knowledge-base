<!--
  创建时间: 2026-06-03
  文件名: PRD编写规范.md
  文件描述: PRD 编写规范详解，涵盖产品需求文档写作标准、AI 产品特殊要求、评审流程与验收方法
...
> 好的 PRD 不是“写得长”，而是“能被不同角色正确理解并推进执行”。

#### 2.3.1 企业最看重的 5 个信号

1. **目标清晰**：为什么做、解决什么问题、成败如何判断
2. **范围明确**：做什么、不做什么、MVP 边界在哪里
3. **流程完整**：正常流程、异常流程、兜底流程都考虑到
4. **验收可执行**：研发、测试、算法、运营都知道如何验收
5. **风险前置**：幻觉、超时、越权、成本、合规均有说明
...
| ------------ | ------------------------------ |
| 页数很多     | 重点不清、优先级混乱           |
| 功能写得很细 | 背景、目标、成功指标不清楚     |
| 正常流程很多 | 异常流程和失败兜底缺失         |
| 技术词很多   | 没有转化为可执行要求和验收口径 |
...
#### 6.4.1 AI产品 PRD 必须额外说明的内容

| 模块 | 为什么必须写 |
|------|-------------|
| AI能力边界 | 防止业务和研发对效果预期不一致 |
| Prompt / 策略说明 | 明确模型行为与输出约束 |
| 评测方案 | AI 功能不能只做功能验收，还要做效果验收 |
| 兜底机制 | AI 错误输出必须有降级、澄清或转人工路径 |
| 成本模型 | 高频场景下，API 成本会直接影响商业可行性 |
| 合规与权限 | 涉及用户数据、日志、内容安全时必须提前说明 |

#### 6.4.2 AI产品 PRD 的评审重点

| 评审角色 | 最关注的问题 |
|---------|-------------|
| 产品 | 目标、范围、成功指标是否清晰 |
| 设计 | 用户心智、异常提示、反馈机制是否合理 |
| 研发 | 接口、边界、状态流转是否明确 |
| 算法 | 模型能力、评测样本、效果目标是否现实 |
| 测试 | 是否有明确可验证的功能与效果验收口径 |
| 运营/客服 | 上线后如何监控、兜底和处理用户反馈 |

#### 6.4.3 AI产品 PRD 最容易漏掉的点

- 模型失败时用户会看到什么
- 用户如何纠错、追问、转人工
- 输出结果是否需要引用来源或置信提示
- 敏感信息是否要脱敏、屏蔽、审计
- 上线后如何收集 Bad Case 并进入迭代
...
- [ ] 能独立写出一份结构清晰、可评审的 PRD
- [ ] 能把 AI 功能需求写成可测试、可验收、可复盘的要求
- [ ] 能定义基础成功指标、风险说明和兜底策略
- [ ] 能组织至少一次 PRD 评审并根据反馈修订
- [ ] 能区分“功能上线”和“AI效果达标”是两件事
