<!--
  创建时间: 2026-06-03
  文件名: FunctionCalling.md
  文件描述: Function Calling 函数调用详解，补充企业级设计模板、风险治理与验收清单
...
- 解释 Function Calling 的工作机制，以及它与普通文本生成的差异
- 设计函数 schema、参数约束、调用策略和返回结果结构
- 比较 OpenAI、Claude、Gemini、DeepSeek 在函数调用上的实现差异
- 为企业应用设计参数校验、异常处理、超时控制和安全边界
- 输出一份可用于研发落地的 Function Calling 方案
...
### 7.1 Function Schema 设计检查表

| 设计项 | 关键问题 | 输出物 |
| ------ | -------- | ------ |
| 函数边界 | 一个函数是否只做一件清晰的事？ | 函数清单 |
| 参数约束 | 参数是否有类型、枚举、必填项和默认值？ | Schema 定义 |
| 结果结构 | 返回结果是否结构化且可被程序消费？ | 返回规范 |
| 安全控制 | 是否限制高风险参数和外部副作用？ | 安全策略 |
| 错误处理 | 超时、参数错误、下游失败如何反馈？ | 错误码规范 |
| 可观测性 | 是否记录调用人、参数、耗时和结果？ | 审计字段 |
...
```json
{
  ...
  "description": "查询订单状态，仅返回当前状态和最近更新时间",
  "parameters": {
    "type": "object",
    "properties": {
      "order_id": {
...
| 误区 | 问题 | 正确做法 |
| ---- | ---- | -------- |
| 函数描述太模糊 | 模型经常选错函数 | 用清晰职责、示例和反例约束调用边界 |
| 一个函数做太多事 | 参数复杂、失败率高 | 保持原子化，再用 Tool Calling 组合 |
| 不校验参数 | 下游系统异常增多 | 在执行前做严格校验和默认值处理 |
| 只处理成功路径 | 失败时模型无法恢复 | 返回结构化错误并设计重试与降级 |
| 忽略审计 | 无法排查误调用和高成本请求 | 记录调用链、耗时、参数和结果摘要 |

---

## 九、阶段验收标准

- [ ] 能设计一个高质量的函数 schema，包含参数、约束和描述
- [ ] 能区分 Function Calling 与更高级 Tool Calling 的边界
- [ ] 能设计参数校验、异常处理、超时与重试机制
- [ ] 能比较主流平台函数调用实现差异
- [ ] 能输出一份可落地的 Function Calling 设计方案
...
- **2026-06-03** 初版完成，涵盖函数调用原理、多平台实现、高级模式与最佳实践
