<!--
  创建时间: 2026-06-03
  文件名: Chunk策略.md
  文件描述: 文档分块策略详解，补充分块决策框架、治理要点与企业级验收标准
...
**能力对标**：本章对应 [能力模型](../00-Roadmap/能力模型.md) 中「AI应用构建力 → 知识库构建能力」核心能力。掌握 Chunk 策略意味着你不只是会切文档，而是能从检索命中率、上下文完整性、成本和维护性角度设计分块方案。
...
#### 3.3.1 选择分块策略前至少要判断 5 件事

| 判断项 | 核心问题 |
|------|---------|
| 文档结构 | 是连续文本、Markdown、代码、表格还是多模态内容 |
| 查询类型 | 更偏事实问答、文档搜索、长文总结还是代码理解 |
| 检索目标 | 更追求命中率还是上下文完整性 |
| 更新频率 | 是否需要增量更新、频繁重建索引 |
| 成本预算 | 能否接受更复杂的离线处理和更高存储成本 |

#### 3.3.2 企业中最常见的分块组合

1. **默认方案**：递归字符分块 + 轻量重叠
2. **高质量方案**：结构化分块或语义分块 + 元数据增强
3. **平衡方案**：父子分块，子块检索、父块生成
4. **高风险方案**：对合同、制度、代码等结构文档做专用分块
...
| 维度 | 关注内容 |
|------|---------|
| **精度与成本** | 小块更精确，大块更省成本，父子分块是常见折中 |
| **延迟与质量** | 简单分块适合实时场景，语义分块更适合离线高质量索引 |
| **通用与专用** | 默认通用策略 + 核心场景专用策略，是多数企业的现实选择 |
| **更新与维护** | 是否支持增量更新，是否易于回滚和重建 |

### 5.2 企业级分块治理重点

1. **文档类型分层治理**：制度文档、FAQ、代码、合同、表格不应一刀切
2. **元数据同步治理**：标题、章节、权限、版本、时间等信息需要与 Chunk 一起管理
3. **坏例子治理**：对命中错误、上下文断裂、重复召回的案例持续复盘
4. **上线后持续调参**：Chunk 大小、重叠、父子映射都应基于效果数据优化

### 5.3 常见分块失误

| 失误 | 表现 | 修复方向 |
|------|------|---------|
| 切得过碎 | 命中片段过短，回答缺上下文 | 增大 chunk 或使用父子分块 |
| 切得过粗 | 检索噪声大，相关内容被淹没 | 减小 chunk 或改结构化分块 |
| 重叠不足 | 关键信息断在边界处 | 提高 overlap 或按语义边界切分 |
| 元数据缺失 | 命中后无法做权限或版本过滤 | 设计完整 metadata 字段 |
...
- [ ] 能解释为什么分块会直接影响召回质量、回答完整性和成本
- [ ] 能针对不同文档类型选择基础分块策略
- [ ] 能理解 chunk size、overlap、结构边界和 metadata 之间的关系
- [ ] 能识别切得过粗、切得过碎、重叠不足等常见问题

### 6.2 进阶标准

- [ ] 能为一个真实知识库场景设计分块策略并说明取舍
- [ ] 能提出父子分块、上下文增强分块或结构化分块的适用边界
- [ ] 能把分块效果纳入 RAG 评测和 Bad Case 复盘闭环
